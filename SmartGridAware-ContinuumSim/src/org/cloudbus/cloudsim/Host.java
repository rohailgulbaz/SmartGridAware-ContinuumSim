...
public class Host {
	...
	public Host(
			int id, double BusyPower, double IdlePower, 
			RamProvisioner ramProvisioner,
			BwProvisioner bwProvisioner,
			long storage,
			List<? extends Pe> peList,
	...
	public boolean vmCreate(Vm vm) {
		if (getStorage() < vm.getSize()) {
			Log.printLine("Message from Host class: [VmScheduler.vmCreate] Allocation of Container #" + vm.getId() + " to Host #" + getId()
					+ " failed by storage");
			return false;
		}
	...
	public boolean ContainerEligibilityForCreation(Vm vm) {
		if (getStorage() < vm.getSize()) {
			Log.printLine("Eligibility of VM #" + vm.getId() + " to Host #" + getId()
					+ " failed by storage");
			return false;
		}
	...
	public double getIdlePowerSum() {
		return IdlePowerSum;
	}
	
	public void setIdlePowerSum(double d) {
		IdlePowerSum=d;
	}
}
