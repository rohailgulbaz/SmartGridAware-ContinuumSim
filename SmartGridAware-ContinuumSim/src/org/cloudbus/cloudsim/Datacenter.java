...
public class Datacenter extends SimEntity {
	...
	/** Onsite Green Energy in watts. */
	private double OnsitePower;
	...
	public Datacenter(
			...
			double schedulingInterval, double Onsite) throws Exception {
		super(name);
	...
	public void processEvent(SimEvent ev) {
		...
		// Resource characteristics inquiry
			...
			case CloudSimTags.CONTAINER_CREATE_ACK:
				processContainerCreate(ev, true);
				break;
	...
	protected void processContainerCreate(SimEvent ev, boolean ack) {
		...
		//System.out.println("getVmAllocationPolicy() "+getVmAllocationPolicy());
		boolean result = getVmAllocationPolicy().allocateHostForVm(vm);

		if (ack) {
			//if vm is successfully created then send its object otherwise null
			if (result) {
				//System.out.println("inside result == true of processContainerCreate in Datacenter");
				int ID = vm.getHost().getDatacenter().getId();
				vm.setDCid(ID);
				...
				//send(vm.getUserId(), CloudSim.getMinTimeBetweenEvents(), CloudSimTags.CONTAINER_CREATE_ACK, vm);
				//sendNow(vm.getUserId(),  CloudSimTags.CONTAINER_CREATE_ACK, vm);
				
				//if vm was successfully created send full data object
				sendNow(vm.getUserId(),  CloudSimTags.CONTAINER_CREATE_ACK, data);
			} else {
				//sendNow(vm.getUserId(), CloudSimTags.CONTAINER_CREATE_ACK, null);
				
				//if vm was not successfully created then send vm=null and send full data object to forward cloudlet too
				data[0]=null;
				sendNow(vm.getUserId(),  CloudSimTags.CONTAINER_CREATE_ACK, data);
			}
	...
	protected void processCloudletSubmit(SimEvent ev, boolean ack) {
		...
		try {
			...
			//Host host = cl.getVm().getHost(); //added this on 20/3/25
			Vm vm = host.getVm(vmId, userId);
			//Vm vm = cl.getVm(); //added this on 20/3/25
	...
	/**
	 *Called to verify the job finishing event 
	 */
	protected void checkJobCompletion(Cloudlet cl) {
	    if (cl != null && cl.isFinished()) {
	        sendNow(getId(), CloudSimTags.JOB_FINISHED, cl);
	    }
	}
		...
		for (int i = 0; i < list.size(); i++) {
			...
			for (Vm vm : host.getVmList()) {
				while (vm.getCloudletScheduler().isFinishedCloudlets()) {
					...
					if (cl != null) {
						Object[] data = new Object[]{cl, vm, host, this};
						sendNow(cl.getUserId(), CloudSimTags.JOB_FINISHED, data);
					}
				}
			}
		}
	 ...
	 * Set Onsite Power.
	 */
	public void setOnsitePower(double OnsitePowerDC) {
		OnsitePower=OnsitePowerDC;
	}
